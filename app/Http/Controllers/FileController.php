    ...
    public function genereExcelReports(Request $request)
    {
        ...
        switch ($report_code) {
            ...
            case 100:
                ...
                $data = DB::table('facturacion_municipales_2 AS FM')
                    ->join('departamentos_visitas_2 AS DV', function ($join) {
                        $join->on('FM.ID_COD_DPTO', '=', 'DV.ID_DEPARTAMENTO');
                    })
                    ->join('municipios_visitas_2 AS MV', function ($join) {
                        $join->on('FM.ID_COD_MPIO', '=', 'MV.ID_MUNICIPIO');
                    })
                    ->leftJoin('recaudo_municipales_2 AS RM', function ($join) {
                        $join->on('FM.ID_FACTURACION', '=', 'RM.ID_FACTURACION');
                    })
                    ->where('FM.PERIODO_FACTURA', $period)
                    ->select([
                        DB::raw('DV.NOMBRE AS DEPARTAMENTO'),
                        DB::raw('MV.NOMBRE AS MUNICIPIO'),
                        DB::raw('FM.CONSECUTIVO_FACT AS FACTURA'),
                        DB::raw('FM.VALOR_FACTURA AS VALOR_FACTURA'),
                        DB::raw('FM.FECHA_FACTURA AS FECHA_FACTURA'),
                        DB::raw('FM.FECHA_ENTREGA AS FECHA_ENTREGA'),
                        DB::raw('FM.FECHA_VENCIMIENTO AS FECHA_VENCIMIENTO'),
                        DB::raw('FM.PERIODO_FACTURA AS PERIODO'),
                        DB::raw("CASE
                            ...
                            END AS ESTADO_FACTURA"),
                        DB::raw('FM.ID_FACTURACION AS ID_FACTURACION'),
                        DB::raw('FM.OBSERVACIONES AS OBSERVACIONES'),
                        DB::raw('SUM(RM.VALOR_RECAUDO) AS VALOR_RECAUDO'),
                        DB::raw('FM.NO_CC_VENCIDAS AS CC_VENCIDAS'),
                        DB::raw('RM.FECHA_PAGO_BITACORA AS FECHA_PAGO_BITACORA'),
                        DB::raw('RM.FECHA_CREACION AS FECHA_CREACION_BITACORA'),
                        DB::raw("CASE
                            ...
                            END AS ESTADO_RECAUDO"),
                        DB::raw('RM.OBSERVACIONES AS OBSERV_RECA'),
                    ])
                    ->groupBy(
                        'MV.NOMBRE',
                        'DV.NOMBRE',
                    ...
                    ->orderBy('DV.NOMBRE', 'DESC')
                    ->orderBy('MV.NOMBRE', 'DESC')
                    ->orderBy('FM.FECHA_FACTURA', 'DESC')
                    ->get();
                return $data;
                break;
            case 9:
                // Reporte Aportes Municipales - Periodo
                $period = $request->input('period');
                $data = DB::select("SELECT DV.NOMBRE AS DEPARTAMENTO,
                MV.NOMBRE AS MUNICIPIO,
                ...
                // delete the default active sheet
                $mySpreadsheet->removeSheetByIndex(0);
                $worksheet1 = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($mySpreadsheet, 'Reporte Aportes Municipales - P');
                $mySpreadsheet->addSheet($worksheet1);
                ...
                $filename = "Reporte Aportes Municipales - Periodo " . $period . ".xlsx";
                ...
                break;
            case 10:
                // Reporte Aportes Municipales municipality
                ...
                AND FM.ID_COD_MPIO = MV.ID_MUNICIPIO
                AND DV.ID_DEPARTAMENTO = MV.ID_DEPARTAMENTO
                AND YEAR(FM.FECHA_FACTURA) = ?
                AND MONTH(FM.FECHA_FACTURA) = ?
                AND MV.ID_DEPARTAMENTO = ?
                AND MV.ID_MUNICIPIO = ?
                ORDER BY DV.NOMBRE, MV.NOMBRE, FM.FECHA_FACTURA DESC
                ...
                // delete the default active sheet
                $mySpreadsheet->removeSheetByIndex(0);
                $sheet_name =  'Reporte Aportes Municip ' . substr(trim(strtoupper($department)), 0, 4);
                $worksheet1 = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($mySpreadsheet, $sheet_name);
                $mySpreadsheet->addSheet($worksheet1);
                ...
                $filename = "Reporte Aportes Municipales " . ucfirst($department) . " - " . ucfirst($municipality) . " - Periodo " . $id_year . $id_month . ".xlsx";
